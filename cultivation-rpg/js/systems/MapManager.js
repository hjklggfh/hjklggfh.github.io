const MapManager = {
    ...
    _cleanup(scene) {
        ...
        for (const t of this.townMarkers) t.destroy();
        this.decorObjects = [];
        this.townMarkers = [];
        this._tileData = null;
    ...
    _createProceduralMap(scene, mapKey) {
        ...
        const random = this._makeRng(mapKey + '_terrain');

        // Initialize tile grid
        this._tileData = new Array(TW * TH).fill(0);
        this._heightData = new Array(TW * TH).fill(0);

        // --- TERRAIN GENERATION ---
        // Base ground with noise
        for (let y = 0; y < TH; y++) {
            for (let x = 0; x < TW; x++) {
                this._tileData[y * TW + x] = theme.ground[Math.floor(random() * theme.ground.length)];
                const ridge = Math.sin((x + y * 0.7) * 0.12) + Math.cos((y - x * 0.4) * 0.09);
                this._heightData[y * TW + x] = Math.max(0, Math.min(3, Math.floor((ridge + 2) * 0.9)));
            }
        }

        // Large rivers are natural barriers.
        this._drawRiver(TW, TH, 5, 28, 94, 38, 2, random);
        this._drawRiver(TW, TH, 58, 4, 64, 94, 2, random);
        ...
        for (let c = 0; c < 8; c++) {
            ...
            for (let y = cy - r; y <= cy + r; y++) {
                for (let x = cx - r; x <= cx + r; x++) {
                    ...
                    if (dist < r) {
                        this._tileData[y * TW + x] = 3;
                        this._heightData[y * TW + x] = 4 + Math.floor((r - dist) / 2);
                    }
                }
            }
        }

        // Forest patches are enterable exploration areas.
        const forestZones = [];
        for (let c = 0; c < 13; c++) {
            const cx = Math.floor(rand(seed++) * (TW - 15)) + 8;
            const cy = Math.floor(rand(seed++) * (TH - 15)) + 8;
            const r = Math.floor(rand(seed++) * 5) + 3;
            forestZones.push({ tx: cx - r, ty: cy - r, tw: r * 2, th: r * 2 });
            for (let y = cy - r; y <= cy + r; y++) {
                for (let x = cx - r; x <= cx + r; x++) {
                    if (x < 0 || x >= TW || y < 0 || y >= TH) continue;
                    const dist = Math.sqrt((x - cx) ** 2 + (y - cy) ** 2);
                    if (dist < r && rand(seed++) > 0.18 && this._tileData[y * TW + x] !== 2 && this._tileData[y * TW + x] !== 3) {
                        this._tileData[y * TW + x] = 4;
                        this._heightData[y * TW + x] = Math.max(this._heightData[y * TW + x], 2);
                    }
                }
            }
        }
        ...
        const towns = [];
        if (mapKey === 'world-1') {
            towns.push({ id: 'qinglan_market', name: '青岚镇', tx: 41, ty: 42, tw: 17, th: 14 });
        } else if (mapKey === 'world-2') {
            towns.push({ id: 'cloud_gate', name: '云门驿', tx: 18, ty: 60, tw: 15, th: 12 });
            towns.push({ id: 'peak_bazaar', name: '玄霄坊', tx: 70, ty: 24, tw: 12, th: 9 });
        } else {
            towns.push({ id: 'ember_outpost', name: '烬月渡', tx: 44, ty: 45, tw: 14, th: 11 });
        }
        ...
        this.townAreas = towns.map(t => ({
            x: t.tx * TS, y: t.ty * TS,
            w: t.tw * TS, h: t.th * TS,
            tx: t.tx, ty: t.ty, tw: t.tw, th: t.th,
            id: t.id, name: t.name
        }));
        this.forestAreas = forestZones.map(f => ({
            x: f.tx * TS, y: f.ty * TS,
            w: f.tw * TS, h: f.th * TS,
        ...
        const collisionTiles = new Set([2, 3]); // large rivers and mountains block movement
        for (let y = 0; y < TH; y++) {
            for (let x = 0; x < TW; x++) {
        ...
        for (const p of portalPositions) {
            ...
            // Visual marker
            const marker = scene.add.circle(p.x + TS, p.y + TS, 20, 0x8bd3ff, 0.56);
            marker.setDepth(15);
            ...
            const label = scene.add.text(p.x + TS, p.y + TS - 30, '传送阵', {
                fontSize: '10px', fill: '#e0fbff', fontFamily: 'monospace'
            }).setOrigin(0.5).setDepth(15);
            this.portalMarkers.push(label);
        }

        for (const town of this.townAreas) {
            const label = scene.add.text(town.x + town.w / 2, town.y - 10, town.name, {
                fontSize: '12px', fill: '#fff0b8', fontFamily: 'monospace',
                stroke: '#3e2f1e', strokeThickness: 3
            }).setOrigin(0.5).setDepth(18);
            this.townMarkers.push(label);
        }
        ...
        const completed = GameState.completedCaves[mapKey] || [];
        for (const cave of allCaves) {
            this.caveEntrances.push({
                ...
                completed: completed.includes(cave.id)
            });
            const isDone = completed.includes(cave.id);
            const alpha = isDone ? 0.36 : (discovered.includes(cave.id) ? 0.72 : 0.14);
            const marker = scene.add.circle(cave.x + TS / 2, cave.y + TS / 2, isDone ? 10 : 14, isDone ? 0x8a7f70 : 0xffd166, alpha);
            marker.setDepth(14);
            this.caveMarkers.push(marker);
            if (!discovered.includes(cave.id) && !isDone) {
                scene.tweens.add({
    ...
    _drawRiver(TW, TH, startX, startY, endX, endY, width, random) {
        ...
        while (Math.abs(x - endX) > 2 || Math.abs(y - endY) > 2) {
            for (let w = -width; w <= width; w++) {
                for (let h = -width; h <= width; h++) {
                    ...
                    if (rx >= 0 && rx < TW && ry >= 0 && ry < TH && Math.sqrt(w * w + h * h) <= width + 0.25) {
                        this._tileData[ry * TW + rx] = 2;
                        if (this._heightData) this._heightData[ry * TW + rx] = 0;
                    }
                }
            }
            const dx = endX - x, dy = endY - y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            const step = 1 + random() * 1.5;
            x += (dx / dist) * step + (random() - 0.5) * 1.2;
            y += (dy / dist) * step + (random() - 0.5) * 1.2;
        }
    },
    ...
    getHeightData() { return this._heightData; },
    ...
    getCurrentTown(worldX, worldY) {
        for (const town of this.townAreas) {
            if (worldX >= town.x && worldX <= town.x + town.w && worldY >= town.y && worldY <= town.y + town.h) {
                return town;
            }
        }
        return null;
    ...
    isForestTile(worldX, worldY) {
        return this.getTileAt(worldX, worldY) === 4;
    },
