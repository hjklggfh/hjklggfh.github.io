const SaveManager = {
    ...
    _normalizeLoadedState() {
        for (const mapId of ['world-1', 'world-2', 'world-3']) {
            if (!GameState.discoveredCaves[mapId]) GameState.discoveredCaves[mapId] = [];
            if (!GameState.completedCaves[mapId]) GameState.completedCaves[mapId] = [];
            if (!GameState.mapPositions[mapId]) GameState.mapPositions[mapId] = { x: 1600, y: 1600 };
        }
        if (!GameState.playerEquipped.weapon && !('weapon' in GameState.playerEquipped)) {
            GameState.playerEquipped.weapon = null;
        }
    },
    ...
    save(slotIndex) {
        const data = {
            ...
            gameState: {
                ...
                completedCaves: JSON.parse(JSON.stringify(GameState.completedCaves)),
    ...
    load(slotIndex) {
        try {
            ...
            GameState.completedCaves = JSON.parse(JSON.stringify(gs.completedCaves || {}));
