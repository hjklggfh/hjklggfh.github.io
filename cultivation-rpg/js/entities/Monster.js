...
Monster.pickForArea = function (mapId, tile) {
    ...
    const picked = { ...(pool[Math.floor(Math.random() * pool.length)] || templates[0]) };
    const rankIdx = mapId === 'world-3' ? 2 : mapId === 'world-2' ? 1 : 0;
    const mult = 1 + rankIdx * 0.45;
    picked.hp = Math.floor(picked.hp * mult);
    picked.atk = Math.floor(picked.atk * mult);
    picked.def = Math.floor(picked.def * mult);
    picked.qiReward = Math.floor(picked.qiReward * mult);
    picked.goldReward = Math.floor(picked.goldReward * mult);
    return picked;
};
