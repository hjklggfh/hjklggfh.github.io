const NPCSpawner = {
    ...
    spawnNPCs(scene, count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!placed) {
                ...
                this._createNPC(scene, town.x + town.w / 2, town.y + town.h / 2);
            }
        }
    },

    _createNPC(scene, x, y) {
        const npcData = NPCDatabase.generateNPC();
    ...
    _isBlocked(scene, wx, wy) {
        ...
        // Blocked by mountains (3), forest (4), water (2)
        if (tile === 2 || tile === 3 || tile === 4) return true;
