...
class Projectile {
    constructor(fireData) {
        ...
        this.isPulse = fireData.isPulse === true;
        this.pulseRadius = fireData.pulseRadius || 70;

        this.lifetime = (this.isAOE || this.isPulse) ? 0.3 : 5;
        this.age = 0;

        // AOE applies instantly
        if (this.isAOE) this._applyAOE();
        // Pulse AOE: instant damage
        if (this.isPulse) this._applyPulse();
    }
    ...
    _applyPulse() {
        ...
        for (var j = 0; j < obstacles.length; j++) {
            ...
            if (Math.sqrt(odx * odx + ody * ody) <= this.pulseRadius) {
